...
# # @parallel InterpV2Ce( ηv, ηc )
# # @parallel InterpV2Ce( ηc[2:end-1,2:end-1,2:end-1], ηv )
# # @parallel InterpV2xyz( ηxy, ηxz, ηyz, ηv )
# #
# @parallel InterpV2C( ηc, ηv )
# @parallel InterpV2C( ηv[2:end-1,2:end-1,2:end-1], ηc )
...
#     @printf("P0 : min = %2.4e --- max = %2.4e\n", minimum( P0[2:end-1,2:end-1,2:end-1])*σc, maximum( P0[2:end-1,2:end-1,2:end-1])*σc)
#     @printf("P  : min = %2.4e --- max = %2.4e\n", minimum(  P[2:end-1,2:end-1,2:end-1])*σc, maximum(  P[2:end-1,2:end-1,2:end-1])*σc)
#     @printf("ηc : min = %2.4e --- max = %2.4e\n", minimum( ηc)*μc, maximum( ηc)*μc)
#     @printf("ηv : min = %2.4e --- max = %2.4e\n", minimum( ηv)*μc, maximum( ηv)*μc)
#     ##########
#     Pin = P[2:end-1,2:end-1,2:end-1]
#     @parallel ComputeStressInvariant( τii, τxx, τyy, τzz, τxy, τxz, τyz )
#     Fs .= τii .- C.*cosd(ϕ) .- Pin.*sind(ϕ)
...
# @parallel_indices (i,j,k) function InterpV2C( ηc, ηv )
#     if (i<=size(ηc,1)-0) && (j<=size(ηc,2)-0) && (k<=size(ηc,3)-0)
#         ηc[i,j,k]  = 1.0/8.0*( ηv[i,  j,  k] + ηv[i+1,j,k  ] + ηv[i,j+1,k  ] + ηv[i,  j,k+1  ] )
#         ηc[i,j,k] += 1.0/8.0*( ηv[i+1,j+1,k] + ηv[i+1,j,k+1] + ηv[i,j+1,k+1] + ηv[i+1,j+1,k+1] )
#     end
#     return nothing
...
#     if (i<=size(εxx,1)) && (j<=size(εxx,2)) && (k<=size(εxx,3))
#         η_e  = Gc[i,j,k]*Δt
#         η_ve = 1.0/(1.0/ηc[i+0,j+0,k+0] + 1.0/η_e)
#         τxx[i+1,j+1,k+1] = 2*η_ve*( εxx[i,j,k] + τxx0[i,j,k]/(2η_e) )
#         τyy[i+1,j+1,k+1] = 2*η_ve*( εyy[i,j,k] + τyy0[i,j,k]/(2η_e) )
#         τzz[i+1,j+1,k+1] = 2*η_ve*( εzz[i,j,k] + τzz0[i,j,k]/(2η_e) )
#     end
