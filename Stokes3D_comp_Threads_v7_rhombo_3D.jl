...
# dt for 3D
using Printf, HDF5, WriteVTK#, LightXML
# using Plots
...
include("RhombicDodecahedron_v2.jl")
...
if restart_from == 0
    ...
    #-----------
    @parallel InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, βc, βr, Gr, ρref, ρr, fits )
    P .= Pini
    #-----------
...
for it=restart_from+1:nt
    #----------- Adaptive Δt
    # Pcpu   = Array(P)
    # Pmid   = Pcpu[(ncx+2)÷2,(ncy+2)÷2,(ncz+2)÷2]
    ...
    if write_out==1 && (it==1 || mod(it, write_nout)==0)
        ...
        @parallel ComputeStressInvariant( εii, εxx, εyy, εzz, εxy, εxz, εyz ) 
        VxC = Array(0.5*(Vx[2:end,:,:]+Vx[1:end-1,:,:]))
        VyC = Array(0.5*(Vy[:,2:end,:]+Vy[:,1:end-1,:]))
        VzC = Array(0.5*(Vz[:,:,2:end]+Vz[:,:,1:end-1]))
        Vc  = (Array(VxC).*Vc, Array(VyC).*Vc, Array(VzC).*Vc)
        # Output for Paraview
        fname = @sprintf("%s/Output%05d", @__DIR__, it)
        vtk_grid(fname, Array(xce[2:end-1]).*Lc, Array(yce[2:end-1]).*Lc, Array(zce[2:end-1]).*Lc) do vtk
            vtk["V"]   = Vc
            vtk["P"]   = Array(P[2:end-1,2:end-1,2:end-1]).*σc
            vtk["η"]   = Array(ηc[2:end-1,2:end-1,2:end-1]).*μc
            vtk["ρ"]   = Array(ρ).*ρc
            vtk["εii"] = Array(εii).*εc
            vtk["τii"] = Array(τii).*σc
            vtk["λc"]  = Array(λc).*εc
        end
        # @parallel ComputeStressInvariant( εii, εxx, εyy, εzz, εxy, εxz, εyz ) 
...
@parallel_indices (i,j,k) function InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, βc, βr, Gr, ρref, ρr, f )
    ri1, ri2, ri3, ro, ar, ari = 0.14*r, 0.10*r, 0.18*r, r, 1.1, 1.4
    sx1, sy1, sz1 =  0.12, 0.02, 0.05
    sx2, sy2, sz2 = -0.16,-0.05,-0.1 
    sx3, sy3, sz3 = -0.13,-0.15, 0.05
    # Vertices
    ...
    if i<=size(ηv,1) && j<=size(ηv,2) && k<=size(ηv,3) 
        X, Y, Z   = xv[i], yv[j], zv[k]
        Gv[i,j,k] = Gr
        βv[i,j,k] = βr
        ηv[i,j,k] = ηr 
        # if (X^2/(ar*ro)^2 + Y^2/ro^2 + Z^2/ro^2) < 1.0  
        if IsInRhombicDodecahedron( X, Y, Z, f )
            ηv[i,j,k] = ηr*100      
            βv[i,j,k] = βr*1.4166   
            Gv[i,j,k] = Gr*1.5*(1.0 + 0.2*(0.5-rand()))     
        end 
        if ((X-sx1)^2/(ari*ri1)^2 + (Y-sy1)^2/ri1^2 + (Z-sz1)^2/ri1^2) < 1.0  
            βv[i,j,k] = βr   
            ηv[i,j,k] = ηr/10.0 
        end  
        if ((X-sx2)^2/(ari*ri2)^2 + (Y-sy2)^2/ri2^2 + (Z-sz2)^2/ri2^2) < 1.0  
            βv[i,j,k] = βr   
            ηv[i,j,k] = ηr/10.0 
        end  
        if ((X-sx3)^2/(ari*ri3)^2 + (Y-sy3)^2/ri3^2 + (Z-sz3)^2/ri3^2) < 1.0  
            βv[i,j,k] = βr   
            ηv[i,j,k] = ηr/10.0 
        end  
    end
