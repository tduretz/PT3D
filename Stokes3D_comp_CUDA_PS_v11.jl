...
dη    = @zeros(ncx+1, ncy+1, ncz+1)
ηref  = @zeros(ncx+1, ncy+1, ncz+1)
...
σ1    = @zeros(ncx+0, ncy+0, ncz+0)
σ2    = @zeros(ncx+0, ncy+0, ncz+0)
σ3    = @zeros(ncx+0, ncy+0, ncz+0)
...
if restart_from == 0
    ...
    #-----------
    @parallel InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, βc, βr, Gr, ρref, ρr, ηref, dη, dηinc )
    P .= Pini
    #-----------
    @parallel UpdateDensity( ρ, ρref, βc, P, Pr, dρ, Pt, dPr )
    @parallel UpdateViscosity( ηv, ηref, P, Pr, dη, Pt, dPr )
...
for it=restart_from+1:nt
    ...
    #-----------  
    @parallel ComputePrincipalStresses( σ1, σ2, σ3, τxx, τyy, τzz, τxy, τxz, τyz, P )
    if do_visu
    ...
    p4  = scatter!(Pin[:].*σc./1e9, τii[:].*σc./1e9, xlabel="P [GPa]", ylabel="τii [GPa]", ylims=(0.2))
    # p4  = plot(P_1d./1e9, ηr_1d,legend=false)
    # p4  = scatter!(Pin[:].*σc./1e9, ηin[:].*μc, xlabel="P [GPa]", ylabel="η [Pa.s]", ylims=(0.5e21, 1.1e22))
    
    p5 = heatmap(xce[2:end-1].*Lc*1e2, zce[2:end-1].*Lc*1e2, σ1[:, (size(σ1,2))÷2, :]'.*σc./1e9, title="σ1 [GPa]", aspect_ratio=1, xlims=(-Lx/2*Lc*1e2, Lx/2*Lc*1e2), c=:jet1 )
    p6 = heatmap(xce[2:end-1].*Lc*1e2, zce[2:end-1].*Lc*1e2, σ2[:, (size(σ2,2))÷2, :]'.*σc./1e9, title="σ2 [GPa]", aspect_ratio=1, xlims=(-Lx/2*Lc*1e2, Lx/2*Lc*1e2), c=:jet1 )
    p7 = heatmap(xce[2:end-1].*Lc*1e2, zce[2:end-1].*Lc*1e2, σ3[:, (size(σ3,2))÷2, :]'.*σc./1e9, title="σ3 [GPa]", aspect_ratio=1, xlims=(-Lx/2*Lc*1e2, Lx/2*Lc*1e2), c=:jet1 )
...
if do_visu gif(anim, "QuartzCoesiteJulia.gif", fps = 6) end
#-----------
return nothing
end

@parallel_indices (i,j,k) function InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, βc, βr, Gr, ρref, ρr, ηref, dη, dηinc  )
...
@parallel_indices (i,j,k) function UpdateViscosity( ηv, ηref, P, Pr, dη, Pt, dPr )
    if i<=size(ηv, 1) && j<=size(ηv, 2) && k<=size(ηv, 3) 
...
@parallel_indices (i,j,k) function ComputePrincipalStresses( σ1, σ2, σ3, τxx, τyy, τzz, τxy, τxz, τyz, P )
