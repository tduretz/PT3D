...
##########
@parallel InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, β, βr, Gr )
P .= Pini
...
@parallel_indices (i,j,k) function InitialCondition( Vx, Vy, Vz, ηv, Gv, βv, ε_BG, ∇V_BG, xv, yv, zv, xce, yce, zce, r, ηr, dρ, dρinc, β, βr, Gr )
    ...
    if i<=size(dρ,1) && j<=size(dρ,2) && k<=size(dρ,3)
        β[i,j,k] = βr/1.2
        if (xce[i+1]^2/(ar*ro)^2 + zce[k+1]^2/ro^2) < 1.0  β[i,j,k]  = βr*1.2 end
        if ((xce[i+1]-0.1)^2/(ari*ri)^2 + (zce[k+1]-0.05)^2/ri^2) < 1.0  β[i,j,k]  = βr end  
...
@views function PlasticCorrection( λ, εxx, εyy, εzz, εxy, εxz, εyz, τxx, τyy, τzz, τxy, τxz, τyz, τxx0, τyy0, τzz0, τxy0, τxz0, τyz0, P, P1, C, cosϕ, sinϕ, sinψ, η_vp, η_ve, η_e, β, Δt, λrel)
    # Plasticity
    τii = sqrt(0.5*(τxx^2 + τyy^2 + τzz^2) + τxy^2 + τxz^2 + τyz^2)
    F   = τii - C*cosϕ - P*sinϕ
    if F>0
        λ1    = F / ( η_ve + η_vp + 1.0./β*Δt*sinϕ*sinψ )
        λ    *= (1.0-λrel)
        λ    += λrel*λ1
        P1   += λ/β*Δt*sinψ
        τii1  = τii - η_ve*λ
